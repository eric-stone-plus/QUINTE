...
use quinte::model::{
    ArbiterVerdict, BRIEF_VERSION, Brief, MULTIMODAL_MODEL, Policy, PrimaryArbiterResponse,
...
fn degenerate_verdict_submission_is_rejected_without_force() {
    ...
    // Schema-valid but obviously a stub: empty residuals + very short texts
    fs::write(
        &stub,
    ...
    // After the guardrail rejects, the run stays waiting and no result is produced
    assert_eq!(
    ...
    // --force is an explicit operator decision: the same stub can finalize
    assert_eq!(
...
fn verdict_schema_error_is_not_reported_as_a_syntax_error() {
    ...
    // Valid JSON, but the residual lacks required fields like evidence_refs
    fs::write(
        &broken,
...
fn amend_rewrites_result_for_a_completed_run() {
    ...
    // result.json and report.md are rewritten by the same finalize path
    ...
    // Manifest summary updated; integrity check passes
    let manifest = store.load_manifest(&run_id).unwrap();
    ...
    // Audit event carries the verdict file sha256; r3/ directory untouched
...
fn amend_enforces_guardrail_and_schema_on_completed_runs() {
    ...
    // Degenerate verdict without --force → rejected, result untouched
    ...
    // Schema-invalid is not waived even with --force
    ...
    // --force waives only the guardrail: the same stub can be written, integrity kept
    assert_eq!(
