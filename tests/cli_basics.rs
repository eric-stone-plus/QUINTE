...
#[test]
fn only_agent_card_runtime_source_uses_the_cargo_product_version() {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    for entry in walkdir::WalkDir::new(root.join("src")) {
        ...
        if text.contains("CARGO_PKG_VERSION") {
            assert_eq!(
                entry.path(),
                root.join("src/a2a/card.rs"),
                "unexpected runtime source depends on the Cargo product version"
            );
        }
    }
}
