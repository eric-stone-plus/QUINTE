...
fn host_spec_preserves_scheduler_and_one_active_boundaries() {
    let legacy = include_str!("../specs/HOST-CLI-LEGACY.md");
    for required in [
        ...
        assert!(
            ...
            "HOST-CLI-LEGACY.md omitted {required:?}"
        );
    }
    let a2a = include_str!("../specs/HOST.md");
    for required in [
        "SendMessage",
        "GetTask",
        "review.result",
        ...
        assert!(a2a.contains(required), "HOST.md omitted {required:?}");
    }
}
