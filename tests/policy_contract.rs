...
fn default_policy_binds_the_fixed_roster_and_models() {
    ...
    assert!(
        policy
            .roster
            .iter()
            .all(|route| route.adapter == "deepseek")
    );
    assert!(
        policy
            .roster
    ...
    assert_eq!(policy.seat.family, "deepseek");
    assert_eq!(policy.seat.provider, "deepseek");
...
fn policy_rejects_route_tuple_drift_and_path_unsafe_ids() {
    ...
    mixed.roster[0].family = "mimo".into();
    assert!(
        validate(&mixed)
            .unwrap_err()
            .to_string()
...
fn spoofed_legacy_seat_id_cannot_bypass_the_production_capability_matrix() {
    ...
    let error = validate(&policy).unwrap_err().to_string();
    assert!(
        error.contains("unsupported production seat family")
            || error.contains("requires provider deepseek")
            || error.contains("unsupported adapter"),
...
fn production_capability_matrix_requires_a_proven_isolated_adapter() {
    validate(&production_policy("deepseek", "deepseek", "deepseek")).unwrap();

    for (family, provider, wrong_adapter) in [
        ("deepseek", "deepseek", "reasonix"),
        ("deepseek", "deepseek", "codex"),
    ...
    // Retired provider families are hard errors, not silent fallbacks.
    for (family, provider, adapter) in [
        ("mimo", "xiaomi", "mimo"),
        ("openai", "openai-api", "codex"),
    ] {
        let error = validate(&production_policy(family, provider, adapter))
            .unwrap_err()
            .to_string();
        assert!(
            error.contains("unsupported production seat family")
                || error.contains("unsupported adapter"),
            "accepted retired family {family}: {error}"
        );
    }
}
