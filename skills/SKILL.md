...
QUINTE has one declared model family per run: five independent R1 paths,
anonymized R2 rechecks on contested outputs only, a Counterpart Arbiter, and
a Primary Arbiter. The effective provider and model names come from the
validated policy and may change between installations. A current provider
binding is not a permanent seat, plan, or protocol promise.
...
An external audit is explicit, optional, and post-completion. It
is not a QUINTE phase and must never be presented as part of automatic QUINTE
completion. Read `specs/HOST-CLI-LEGACY.md` for the in-service CLI host
receipts, `specs/HOST.md` for the A2A v1.0 host contract, and
`specs/PROTOCOL.md` only when protocol interpretation is required.
