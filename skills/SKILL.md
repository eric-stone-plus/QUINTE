...
4. Branch on the returned `status`, not the exit code alone. A default detached
   run returns `queued`; exit `0` with `waiting_hm` is a handoff, not completion.

## Hermes hm Handoff

When status is `waiting_hm`:

1. Run `quinte hm request <run-id> --json`.
...
4. Run `quinte hm submit <run-id> --verdict <file> --json`. The CLI copies all
...
`hm_approved`, phase instruction, route override, or claimed producer identity;
only the scheduler state and `hm submit` handshake control progression.
