...
fn omp_credential_status(home: &Path) -> (bool, String) {
    ...
    for name in ["config.yml", "models.yml", "agent.db"] {
        ...
        if !is_regular_nonempty_file(&path) {
            return (
                false,
                format!(
                    "Oh-My-Pi credential state missing or invalid: {}",
                    path.display()
                ),
            );
        }
    ...
    match validate_omp_database(&agent_dir.join("agent.db")) {
        Ok(()) => (
            true,
            "Oh-My-Pi config/model route and active credential database are available".into(),
        ),
        Err(error) => (false, format!("Oh-My-Pi credential database invalid: {error:#}")),
    }
}
...
```json
{"lane_output_version":"0.1.1","task_restatement":"cut off""#;
        let stream = serde_json::json!({"type": "content", "content": content}).to_string();
    ...
    fn codewhale_parser_never_falls_back_past_a_truncated_final_lane_output() {
        ...
        let content = format!(
            "{old}\n```json\n{{\"lane_output_version\":\"0.1.1\",\"task_restatement\":\"final but truncated\""
        );
        let stream = serde_json::json!({"type": "content", "content": content}).to_string();
    ...
    fn codewhale_retry_completion_requires_valid_events_and_retryable_content() {
        ...
        let schema_invalid = format!(
            ...
            serde_json::json!({
                "type": "content",
                "content": "{\"lane_output_version\":\"0.1.1\",\"task_restatement\":\"missing fields\"}"
            }),
            serde_json::json!({"type": "metadata", "meta": {"status": "completed"}}),
            serde_json::json!({"type": "done"})
        );
        ...
        let truncated = format!(
            ...
            serde_json::json!({
                "type": "content",
                "content": "```json\n{\"lane_output_version\":\"0.1.1\",\"verdict\":\"cut off"
            }),
            serde_json::json!({"type": "metadata", "meta": {"status": "completed"}}),
            serde_json::json!({"type": "done"})
        );
