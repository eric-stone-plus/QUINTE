...
mod tests {
    ...
    fn deepseek_invocation_encodes_staged_attachments_as_image_parts() {
        ...
        assert!(prompt.contains(
            "`waived` and `closed` belong ONLY to closure_state, never to disposition"
        ));
        assert!(prompt.contains(
