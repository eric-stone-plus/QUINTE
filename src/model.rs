...
pub const TEXT_MODEL: &str = "deepseek-v4-pro";
pub const MULTIMODAL_MODEL: &str = "deepseek-v4-pro";
...
pub struct TrialManifest {
    ...
    /// Per-run observables from the R1 contestation predicate: how much
    /// the manufactured perspectives actually diverged. Optional and
    /// additive — older results without it stay valid.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub observed_contestation: Option<ObservedContestation>,
}

/// R1 divergence observables (ASSUMPTIONS-REVIEW measurement plan):
/// a contested rate stuck at 0 (or 1) signals the same-family
/// five-school hypothesis needs rework.
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ObservedContestation {
    pub r1_lane_count: usize,
    pub contested_lane_count: usize,
    pub contested_rate: f64,
    /// normalized verdict -> lane count
    pub verdict_distribution: std::collections::BTreeMap<String, usize>,
    /// residual id -> number of lanes attesting it
    pub residual_attestations: std::collections::BTreeMap<String, usize>,
    pub r2_skipped: bool,
}
