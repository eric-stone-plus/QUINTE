    ...
    Policy {
        ...
        roster: ["A", "B", "C", "D", "E"]
            ...
            .map(|(letter, perspective)| {
                production_route(
                    &format!("Party {letter}"),
                    &format!("deepseek-{}", letter.to_ascii_lowercase()),
                    perspective,
                    &seat,
                )
            })
...
fn validate_with_options(policy: &Policy, allow_fake: bool) -> anyhow::Result<()> {
    ...
    for (index, route) in policy.roster.iter().enumerate() {
        ...
        // Five distinct, declared schools (SINGLE-VENDOR-DOCTRINE): a
        // duplicate or empty perspective fails preflight — the school
        // is the roster's diversity axis under the single-family model.
        // Legacy v1 policies predate perspectives and stay exempt until
        // cutover (conformance item 10).
        if !policy.legacy_v1_source {
            let perspective = route.perspective.trim();
            if perspective.is_empty() {
                bail!("roster school perspective for {party_id} must be declared (non-empty)");
            }
            if !perspectives.insert(perspective) {
                bail!("roster school perspectives must be pairwise distinct (duplicate: {perspective})");
            }
        }
        let fake = allow_fake
            && matches!(
                route.adapter.as_str(),
                "fake" | "fake_envelope" | "fake_codewhale"
            );
    ...
    let fake_arbiter = allow_fake
        && matches!(
            ...
            "fake" | "fake_envelope" | "fake_arbiter"
        );
...
fn validate_adapter_capability(
    ...
    if legacy_v1_source {
        if !matches!(
            ...
            bail!(
                "{} uses an adapter incompatible with the legacy v1 roster",
                route.party_id
            );
        }
        return Ok(());
