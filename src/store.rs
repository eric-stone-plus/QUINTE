...
impl Store {
    ...
    /// Bypass the terminal silent-skip guard in save_manifest. Only for paths
    /// that already hold the run lock and have a legitimate reason to update a
    /// terminal run's metadata in place (currently only primary-arbiter amend
    /// rewriting result_sha256).
