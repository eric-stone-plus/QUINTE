...
pub use crate::contract::{
    ARBITER_VERDICT_SCHEMA, BRIEF_SCHEMA, HOST_RECEIPT_SCHEMA, LANE_OUTPUT_SCHEMA,
    LEGACY_BRIEF_SCHEMA, LEGACY_HM_RESPONSE_SCHEMA, LEGACY_RESULT_SCHEMA,
...
pub fn validate_value(value: &Value, schema: &str) -> anyhow::Result<()> {
    ...
    for schema in crate::contract::AUXILIARY_SCHEMA_DOCUMENTS {
        let document: Value =
            serde_json::from_str(schema).context("auxiliary finance JSON schema is invalid")?;
        let id = document
            .get("$id")
            .and_then(Value::as_str)
            .context("auxiliary finance schema has no $id")?
            .to_string();
        if !embedded.iter().any(|(existing, _)| existing == &id) {
            embedded.push((id, document));
        }
    }
