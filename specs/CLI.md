...
binding DeepSeek, executed by the native in-process adapter over the
OpenAI-compatible chat-completions API. `doctor` checks all seven same-family
execution bindings and the selected provider environment pair.
...
quinte host serve [--bind 127.0.0.1:8801] [--token-env VAR] [--json]
...
reconciliation without taking ownership of QUINTE phases. See
[HOST-CLI-LEGACY.md](HOST-CLI-LEGACY.md); the A2A v1.0 replacement contract is
[HOST.md](HOST.md).
...
selects the multimodal model. The in-process DeepSeek adapter sends each image
as a base64 `image_url` content part in the chat-completions request. The
...
Container integrations select one key variable and one base URL variable
through `QUINTE_PROVIDER_KEY_ENV` and `QUINTE_PROVIDER_BASE_URL_ENV`. The
selected names must match the seat family: `DEEPSEEK_API_KEY` and
`DEEPSEEK_BASE_URL`. Only that pair is consulted for a lane's in-process
provider call. URLs must be configured HTTPS endpoints without whitespace or
`.invalid`.
