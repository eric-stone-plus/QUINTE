# QUINTE Host Invocation Contract (Legacy CLI Surface)

> **Legacy document.** This contract is the implemented host boundary of the
> current 0.2.x runtime (`host preflight/start/status/inspect/reconcile`).
> It is superseded by [HOST.md](HOST.md), the A2A v1.0 endpoint contract,
> once that endpoint ships — which is deliberately deferred until the
> provider/model re-selection is decided. Until then this file remains the
> operational reference for the CLI host surface; its implementation lives
> in `src/host.rs` and `schemas/host-invocation.schema.json`.

This document defines the stable boundary for a non-interactive host such as
Codex, a CI coordinator, or a local campaign runner. The host owns
...
The coordinator's environment must also carry the provider credential
indirection that preflight resolves: `QUINTE_PROVIDER_KEY_ENV` and
`QUINTE_PROVIDER_BASE_URL_ENV`, plus the variables they name (for example
`XIAOMI_API_KEY` / `XIAOMI_BASE_URL`). Login-shell exports (for example from
`~/.bashrc`) are not visible to systemd user services, so a launch that works
interactively can fail preflight under systemd with "required adapters
unavailable". Pass the variables through an `EnvironmentFile=` with mode 0600
or an equivalent secret store; do not bake credentials into unit files,
briefs, or receipts.

A run whose worker died before preflight — for example, reaped by a
control-group coordinator exiting — stays `Queued` indefinitely; nothing
re-runs preflight for it. Treat a run that remains `Queued` far longer than a
normal preflight (on the order of 15 minutes) as a dead launch: cancel it with
`quinte cancel RUN_ID --json` and submit a fresh run. Reconcile binds the
durable state but does not retry a dead launch.
...
- The host contract does not make HIGHBALL, Codex, or another host part
