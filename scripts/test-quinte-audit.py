...
output = os.environ.get("FAKE_AUDIT_OUTPUT", "{}")
if Path(sys.argv[0]).name == "reasonix":
    print(json.dumps({
        "is_error": False,
        "result": output,
...
class QuinteAuditTests(unittest.TestCase):
    ...
    def test_quinte_home_environment_is_used(self):
        ...
        process = self.run_audit(
            self.run_id, "--auditor", "omp", "--model", "mimo/mimo-v2.5-pro",
    ...
    def test_reasonix_default_identity_and_tool_restrictions_are_explicit(self):
        process = self.run_audit(self.run_id, "--home", str(self.home), "--json")
        self.assertEqual(process.returncode, 0, process.stderr)
        pointer = json.loads(process.stdout)
        self.assertEqual(pointer["auditor"], "reasonix")
        self.assertEqual(pointer["model"], "deepseek-v4-pro")
        self.assertEqual(pointer["family"], "deepseek")
        arguments = json.loads((self.args_dir / "reasonix.json").read_text())
        self.assertEqual(arguments[arguments.index("--model") + 1], "deepseek-v4-pro")
        self.assertIn("--allowed-tools", arguments)
        self.assertEqual(arguments[arguments.index("--allowed-tools") + 1], "")
